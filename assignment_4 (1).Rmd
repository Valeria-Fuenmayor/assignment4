...
author: |
  ...
  | https://github.com/Valeria-Fuenmayor/assignment4
date: April 2023
urlcolor: purple
linkcolor: purple
output: 
...
treat <- datasummary((mean + median + sd + min + max) ~ Factor(treat) * 
                       (growth + rgdp60), data=GrowthSW)
treat
```
...
As far as I can notice, we have the same results. What we have done is calculated the mean, median, sd, min, and max for the variables growth and rgdp60 for two groups of the countries listed in the dataset GrowthSW. 
As far as I can notice, we have the same results. What we have done is calculated the mean, median, sd, min, and max for the variables growth and rgdp60 for two groups of the countries listed in the dataset GrowthSW. 
...
The p-value is 0.06871. This means that there is a 0.06871% of the null-hypothesis being true. However, since the p-value is greater than the significance level of 0.05 than we would fail to reject the null hypothesis.
...
The variable 'rgdp60' is the value of GDP per capita in 1960, converted to 1960 US dollars. The main purpose of this variable being included is because it indicates a measurement of how the value of output a person has as well as being an indicator of the performance of the economy.
